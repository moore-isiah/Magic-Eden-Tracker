...
const WalletTracker = () => {
  const [wallet, setWallet] = useState(
    "AC1r4GYj1TZ62ASZS4VT6GnNtMzoD78wYNWfxJ3Uzbxy"
  );
  ...
  const {
    ...
    ["transactions"],
    async ({ pageParam = 1 }, wal = wallet) => {
      const walletRes = await fetch(
        `https://api-mainnet.magiceden.dev/v2/wallets/${wal}/activities?offset=${
          (pageParam - 1) * 50
        }&limit=50`
      );
    ...
    },
    {
      getNextPageParam: (lastPage, page) =>
        lastPage.length === 50 ? page.length + 1 : undefined,
    }
  );

  //using react-query fetch all of the sent offers
  const { data: bidOutData, refetch: refetchBidsOut } = useQuery(
    ["bidOutData"],
    async ({}, wal = wallet) => {
      const bidOutRes = await fetch(
        `https://api-mainnet.magiceden.dev/v2/wallets/${wal}/offers_made`
      );
  ...
  const { data: bidInData, refetch: refetchBidsIn } = useQuery(
    ["bidInData"],
    async ({}, wal = wallet) => {
      const bidInRes = await fetch(
        `https://api-mainnet.magiceden.dev/v2/wallets/${wal}/offers_received`
      );
  ...
  async function newWallet() {
    const res = await fetch(
      `https://api-mainnet.magiceden.dev/v2/wallets/${userInput}/tokens`
    );
    ...
    <div className="containers" onScroll={handleScroll}>
      ...
      <div className="scroll-container">
        {allData?.pages.map((page) => {
          let temp = page?.filter(
            (arg) => arg.type === "buyNow" && arg.buyer === wallet
          );
  ...
  const sells = (
    <div className="containers" onScroll={handleScroll}>
      ...
      <div className="scroll-container">
        {allData?.pages.map((page) => {
          let temp = page.filter(
            (arg) => arg.type === "buyNow" && arg.seller === wallet
          );
          let final = temp.map((item) => {
            return <ScrollItem tokenMint={item.tokenMint} price={item.price} />;
          });
  ...
  const bidsOut = bidOutData?.map((item) => (
    <ScrollItem tokenMint={item.tokenMint} />
  ));
  ...
  return (
    <div className="wallet-tracker">
      ...
      <div className="containers submit-section">
        <input
          type="textbox"
          className="wallet-textbox"
          value={userInput}
          onChange={(event) => setUserInput(event.target.value)}
          placeholder="Enter New Solana Wallet"
        />
        <button className="submit-wallet-button" onClick={() => newWallet()}>
          Submit Wallet
        </button>
        <p>
          Use the default wallet to test the website or submit your own and
