...
export default function Portfolio() {
  ...
  const { data, refetch: refetchCollections } = useQuery(
    [`portfolio${wallet}`],
    async ({}, wal = wallet) => {
      let final = [];
      const res = await fetch(
        `https://api-mainnet.magiceden.dev/v2/wallets/${wal}/tokens`
      );
  ...
  const Collections = () => {
    const CollectionHeader = (props) => {
      const { data: stats } = useQuery(
        ...
        async ({}, name = props.show.collection) => {
          const statsRes = await fetch(
            `https://api-mainnet.magiceden.dev/v2/collections/${name}/stats`
          );
      ...
      let meFees = 0.02;
      let royalty = props.show.sellerFeeBasisPoints * 0.0001;
      ...
      return (
        <div className="collection-header">
          ...
          <p>
            Floor:
            <img className="port-sol" src={solLogo} />
            {twoRound(floor)}
          </p>
          <div className="floor-container">
            ...
            <p>
              With ME Fees:
              <img className="port-sol" src={solLogo} />
              {twoRound(totalFloor - totalFloor * meFees)}
            </p>
            <p>
              With Fees and Royalties:
              <img className="port-sol" src={solLogo} />
              {twoRound(totalFloor - totalFloor * (meFees + royalty))}
            </p>
          </div>
        </div>
      );
  ...
  return (
    <div className="portfolio">
      <div className="port-enter-section">
        ...
        />
        <button className="port-button" onClick={newWallet}>
          Submit
        </button>
      </div>
      <Collections />
